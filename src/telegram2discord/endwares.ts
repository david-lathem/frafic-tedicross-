...
export const relayMessage = (ctx: TediCrossContext) => {
	...
	R.forEach(async (prepared: any) => {
		try {
			...
			const searchString = "Join VIP Now";

			const lowerInputString = prepared.text.toLowerCase();
			...
			const updatePieces = ["update", "target", "hit", "profit"];
			...
			if (isUpdateMessage && parts.length > 1) {
				...
				const pattern = /(\d+\w*?)\s+Update:\s+(\w+).*?Target\s+(\d+)\s+Hit\s*-\s*([\d.]+).*?(\d+)%\s+Profit/i;
				...
				if (match) {
					...
					function getRandomBetween(min = 0.003, max = 0.009) {
						return Math.random() * (max - min) + min;
					}

					const randomNum = getRandomBetween()
						.toString()
						.match(/^-?\d+(?:\.\d{0,3})?/)[0];
					...
					prepared.text = `🚨 $${ticker} - Target ${targetAmount} Reached\n\n🆕 Update #${updateNumber}\n\n🧨 Hit: ${(numberedHit - randomNum).toString().match(/^-?\d+(?:\.\d{0,3})?/)[0]}\n\n📈 Profit: +${profit}%`;
				}
				...
				// 		const isTarget = loweredLine.includes("target") && loweredLine.includes("hit");
				// 		return isTarget ? `> ${line}\n> ` : `> ${line}`;
				// 	})
				// 	.join("\n")
				// 	.replace("✓", " ") // Remove the checkmark
				// 	.replace("-", ":") // Change the dash to a colon after 'Hit'
				// 	.replace("👇", "👉 ") // Change the dash to a colon after 'Hit'
				// 	.replace("💸", "💰") // Replace the money bag symbol
				// 	.replace("💸", "") // Replace the money bag symbol
				// 	.replace("💲", "💰") // Replace the money bag symbol
